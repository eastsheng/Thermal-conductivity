...
elif heatflux_direction=3,namely, z direction is the heat flux direction,nanotube
'''
class ThermalConductivity(object):

	def read_size(self,relax_data,i,thickness,r=0,heatflux_direction=1):
		...
		with open(self.relax_data,'r')as data:
			...
			elif heatflux_direction==3:
				self.area = (2*np.pi*self.r)*self.thickness*(1e-18)
	...
	def temp_grad(self,tempfile,number_layers,number_fixed,number_bath,fit_factor,Plot=True):
		...
		elif self.heatflux_direction==3:
			thickness_eachlayer = self.system_size_z/self.number_layers
